...
const EditResume = () => {
  ...
  const setupdateeducation =(id) =>{
    router.push(`/resumeForms/UpdateAducation/${id}`);
  }
 
  const setupdateIntership = (id) => {
    router.push(`/resumeForms/UpdateIntership/${id}`);
  }
  const setupdateJob = (id) => {
    router.push(`/resumeForms/UpdateJobs/${id}`);
  }
  const setupdateresponsibilitie = (id) => {
    router.push(`/resumeForms/UpdatePositions/${id}`);
  }
  const setupdateCourses = (id) => {
    router.push(`/resumeForms/UpdateCourses/${id}`);
  }
  const setupdateproject = (id) => {
    router.push(`/resumeForms/UpdateProjects/${id}`);
  }
  const setupdateskill = (id) => {
    router.push(`/resumeForms/UpdateSkills/${id}`);
  }
  const setupdateaditional = (id) => {
    router.push(`/resumeForms/UpdateAditional/${id}`);
  }
  ...
  const downloadPDF = async () => {
    ...
    try {
      // Import html2pdf.js dynamically on the client side
      const html2pdf = (await import('./html2pdf')).default;
  
      // Use html2pdf as usual
  ...
  return (
    <Layout>
      ...
      <div className="w-full border-[1px] border-slate-400   px-4 py-3 flex justify-between sm:px-16 sm:py-7">
        <div>
          ...
          <h4 className="text-zinc-600 text-xs sm:pb-1 sm:text-sm"><span>Contact:</span> {" "}
              ...
              {resume?.internships?.map((internship) => (
                <div className="flex py-1 border-b-[1px] mb-2 justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Description: {internship.description}</p> 
                  </div>
                  <div>
            ...
            <div>
              {resume?.responsibilities.map((responsibilitie) => (
                <div className="flex justify-between border-b-[1px] border-slate-300 ">
                  <ul>
                    <li className="text-sm text-slate-500">{responsibilitie.description}</li>
                  </ul>
                  <div>
            ...
            <div>
              {resume?.accomplishments.map((accomplishment) => (
                <div className="flex py-1 justify-between">
                  <div>
                    <li>
                      <p className="text-sm text-slate-500">{accomplishment.description}</p>
                    </li>
                  </div>
                  <div>
                  <MdOutlineDelete onClick={() => Deletaddi(accomplishment.id)}  className="h-5 inline-block ms-8 mt-1"/>
                  </div>
                </div>
              ))}
            </div>
          }
          <button className="text-sky-500 text-[8px] sm:text-sm" onClick={() => setaddaditional()}>
    ...
    <button className=" px-3 py-2 mb-10 bg-red-800 text-white" onClick={() => downloadresume()}>Downlode PDF</button>
    </div>
    </div>
    } 
